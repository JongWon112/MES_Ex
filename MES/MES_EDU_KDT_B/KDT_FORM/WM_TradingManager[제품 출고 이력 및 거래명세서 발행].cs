    ...
    public partial class WM_TradingManager : DC00_WinForm.BaseMDIChildForm
    {
        ...
        private void DoFind()
        {
            ...
            try
            {
                ...
                rtnDtTemp = helper.FillTable("06WM_TradingManager_S1", CommandType.StoredProcedure
		...
		private void grid1_AfterRowActivate(object sender, EventArgs e)
		{
            ...
            try
            {
                ...
                string sInvoNo     = Convert.ToString(grid1.ActiveRow.Cells["TRADINGNO"].Value);


                rtnDtTemp = helper.FillTable("06WM_TradingManager_S2", CommandType.StoredProcedure
                                                                   , helper.CreateParameter("PLANTCODE", sPlantCode )
																   , helper.CreateParameter("INVONO",    sInvoNo)
																   );
                this.ClosePrgForm();
        ...
        private void btnInvPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                int iCustCount = 1;              // 업체의 개수                                                     //  업체 개수 변수, 업체명 변수(컨버트 형변환) 생성
                string sCustName = Convert.ToString(rtnDtTemp.Rows[0]["CUSTNAME"]); // 업체 명                      // 
                                                                                                                    //  for문 로직 들어감 row 카운터
                for (int i = 1; i < rtnDtTemp.Rows.Count; i++)                                                      //  
                {                                                                                                   //  if 업체명변수 의 업체명과 같지 않으면 로직 안으로 들어온다  
                    if (sCustName != Convert.ToString(rtnDtTemp.Rows[i]["CUSTNAME"]))  //LG 애플 도요타             //  LG 애플 도요타
                    {                                                                                               //  
                ...
                {                                                                                                   //
                    ...
                    {                                                                                               //
                        iDataTableIndexs++;                                                                         //
                        dtReport[iDataTableIndexs] = new DataTable();                                               //
                        dtReport[iDataTableIndexs] = rtnDtTemp.Clone();                                             //
                        sCustName = Convert.ToString(rtnDtTemp.Rows[i]["CUSTNAME"]);                                //
                    }                                                                                               //
                    dtReport[iDataTableIndexs].ImportRow(rtnDtTemp.Rows[i]);                                        //
                }
